...
async fn read_sources(
    ...
    for attempt in 0..3 {
        ...
        let result = llm::call(ctx, system, input).await.and_then(|s| {
...
pub async fn outline(ctx: &RunContext, system: &str) -> Result<Outline> {
    ...
    for attempt in 0..3 {
        ...
        let input = json!({"purpose":ctx.snapshot.task.direction,"language":ctx.snapshot.task.language,
            "source_brief":brief,"evidence":evidence,"max_diagrams":ctx.snapshot.task.max_diagrams,
