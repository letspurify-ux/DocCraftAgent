...
목차를 정하기 전에 선택 범위의 전체 소스 청크를 나누어 읽습니다. 파일 수 상한과 500개 목록 제한은 없으며, 개별 파일의 크기·제외 규칙과 실행의 토큰·시간 한도는 적용됩니다. 읽은 묶음의 역할·입력·상태 변화·결과·미확인 연결을 계층적으로 종합하고, 그 결과와 실제 근거를 바탕으로 문서 목적에 맞는 질문을 정합니다. 질문마다 담당 섹션을 배정한 뒤 누락·중복·순서·범위를 검토하고 최대 2회 보정합니다.
