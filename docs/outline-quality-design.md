...
목차의 폭은 축약 트리의 브랜치 수를 따른다. 루트 요약 하나로 계획하면 소스가 아무리 커져도 findings 상한과 요약 바이트 예산이 절 수를 정해, 큰 프로젝트가 같은 크기의 더 성긴 문서가 된다. 본문 작성은 이미 `section_memory`로 리프 관찰을 되살리므로 그 한계는 "절이 무엇을 말할 수 있는가"가 아니라 "어떤 절이 존재할 수 있는가"에만 있었다. 트리는 팬인 4로 접히므로 소스가 커져도 상단 레벨의 크기는 대략 1, 4, 16, 64에 머문다. 절대 임계값으로 레벨을 고르면 리프 52개가 13개 브랜치, 520개가 9개 브랜치로 **더 큰 소스가 더 적은 브랜치**로 서술된다. 그래서 목표 브랜치 수는 리프 수를 따라가고, 절 상한도 고정값이 아니라 브랜치 수를 따른다. 저장된 목차를 브랜치 정보 없이 다시 검증할 때는 가장 넓은 상한을 쓴다 — 계획 시점에 유효했던 목차가 다음 읽기에서 거부되면 안 된다. 브랜치 목록은 **소스에 무엇이 있는지**이지 문서가 절을 빚진 목록이 아니다. 브랜치 뷰는 목적을 모르는 소스 읽기 트리에서 나오므로, 목적이 묻지 않는 브랜치는 파일이 아무리 많아도 절을 갖지 않고 `out_of_scope`로 이름만 남긴다. 브랜치가 바꾸는 것은 상한이다. 큰 소스에 좁은 목적이면 짧은 문서가 정답이며 방을 못 채운 실패가 아니다. 계획 요청은 루트 아래로 내려가 목표 이상인 레벨을 골라, 각 브랜치가 다루는 주제와 파일 수를 공유 예산 안에서 고르게 나눠 싣는다. LLM 호출을 추가하지 않고 체크포인트만 읽는다.
...
- 명시한 장 수와 그림 수의 충돌을 계획 전에 표시한다. 장 수 지원 범위는 1~32개이며, 범위 밖의 명시적 요청을 조용히 무시하지 않는다. 32개는 목표가 아닌 상한이며, 독자 목표와 원문으로 뒷받침되는 흐름을 충분히 설명하는 최소 개수를 선택한다. 겹치거나 내용이 적은 주제는 합치고 보조 설명은 하위 제목으로 구성한다.
