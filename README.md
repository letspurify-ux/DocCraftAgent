...
생성 방향에서 독자와 달성할 일을 파악한 뒤, 전체 설명 흐름과 공통 용어를 먼저 계획합니다. 각 절은 독자가 해결할 질문과 다음 절로 이어지는 결과를 갖습니다. 예를 들어 관리자 문서는 시스템의 전체 동작을 이해한 뒤 준비 → 첫 실행 → 결과 확인 → 운영·문제 해결로 읽을 수 있도록 구성합니다. 소스 파일명은 근거 검색에 사용하고, 필요하지 않은 내부 함수 목록은 본문에서 줄입니다.
...
본문 근거는 짧은 각주로 표시하고 원본 경로·행 범위·근거 ID는 참조 목록에 보존합니다. 미해결 경고는 상단에 요약하고 상세는 문서 끝과 라이브러리의 접을 수 있는 목록에서 확인합니다. 이 구성 계획은 새 실행에 적용하며, 기존 체크포인트와 생성 파일을 자동 변환하지 않습니다.
