...
/// Why a file stays in the retrieval index but is left out of the whole-source
/// reading, or `None` when it should be read.
///
/// The reading pass is the one place every passage goes to the model, and lock
/// files, minified bundles and generated clients are large, carry no design a
/// reader needs explained, and are already described by the sources that
/// produce them. In this repository they were a quarter of the tracked bytes.
/// They are still indexed, so a section that asks for one by name retrieves it.
/// `head` is the start of the file, where generators leave their marker.
pub fn reading_exclusion(path: &str, head: &str) -> Option<&'static str> {
    let name = Path::new(path)
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or_default()
        .to_lowercase();
    const LOCKFILES: [&str; 18] = [
        "package-lock.json",
        "npm-shrinkwrap.json",
        "yarn.lock",
        "pnpm-lock.yaml",
    ...
    if [".min.js", ".min.mjs", ".min.css", ".js.map", ".css.map"]
        .iter()
    ...
    if name.contains(".generated.")
        ...
        || [".pb.go", "_pb2.py", "_pb2_grpc.py", ".g.dart"]
            .iter()
            .any(|suffix| name.ends_with(suffix))
    {
...
mod tests {
    ...
    #[test]
    fn lock_generated_and_minified_files_are_indexed_but_not_read() {
        for (path, reason) in [
            ("/p/frontend/package-lock.json", "lockfile"),
            ("/p/Cargo.lock", "lockfile"),
            ("/p/go.sum", "lockfile"),
            ("/p/static/app.min.js", "minified"),
            ("/p/static/app.js.map", "minified"),
            ("/p/frontend/src/api.generated.ts", "generated"),
            ("/p/proto/service_pb2.py", "generated"),
        ] {
            assert_eq!(reading_exclusion(path, ""), Some(reason), "{path}");
        }
        // Generators mark the files they write; the marker is only trusted at
        // the head of a file.
        assert_eq!(
            reading_exclusion(
                "/p/src/client.ts",
        ...
        let late_marker = format!("{}// do not edit", "x".repeat(700));
        assert_eq!(reading_exclusion("/p/src/main.rs", &late_marker), None);
        // Ordinary sources, manifests and documents are read.
        for path in [
            "/p/src/main.rs",
            "/p/Cargo.toml",
            "/p/frontend/package.json",
            "/p/frontend/openapi.json",
            "/p/README.md",
            "/p/src/lockfile.rs",
            "/p/src/minify.js",
        ] {
            assert_eq!(reading_exclusion(path, "fn main() {}"), None, "{path}");
        }
    }
