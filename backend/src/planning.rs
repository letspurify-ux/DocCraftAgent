...
use crate::{
    ...
    db, editorial,
    findings::{
        Discovery, Finding, MAX_FINDINGS, MAX_SECTION_ANCHORS, SourceBrief, anchors, bounded_text,
        resolve_ids, uncited, with_limits,
    },
    llm,
    model::{BranchExclusion, Evidence, Outline, OutlineReview, SectionPlan},
    runner::{fatal, is_budget, outline_diagram_error},
    source,
};
...
/// The outline a previous attempt at this run already settled on, if any.
///
/// Checked before the reading stages rather than inside `outline`, because a run
/// that already has an outline must not pay to read its source again. Runs made
/// before whole-source reading existed keep their outline and their numbered
/// drafts this way.
pub async fn saved_outline(ctx: &RunContext) -> Result<Option<Outline>> {
    match db::load_checkpoint(&ctx.pool, &ctx.id, "outline").await? {
        Some(saved) => Ok(Some(serde_json::from_value(saved)?)),
        None => Ok(None),
    }
}

/// Turns a purpose-focused reading of the source into a reviewed outline.
///
/// Generates a plan, reviews it for gaps, duplication and order, and corrects it
/// at most twice. Returns only once the review reports no major issue; a run
/// configured to preview its outline stops here for approval instead.
pub async fn outline(ctx: &RunContext, system: &str, discovery: Discovery) -> Result<Outline> {
    ...
    loop {
        ...
        review
            .issues
            .extend(unassigned_issues(&coverage, &review.issues));
...
mod tests {
    use super::*;
    use crate::findings::{
        ACCEPTED_EVIDENCE_IDS, ACCEPTED_FINDINGS, ACCEPTED_UNCERTAINTIES, FindingKind,
        MAX_EVIDENCE_IDS, MAX_UNCERTAINTIES, pack_evidence, validate_brief,
    };
    ...
    fn every_branch_is_covered_excluded_or_reported_unassigned() -> Result<()> {
        ...
        let both = json!({"reader_goal":"goal","storyline":"story","sections":[
            ...
            "excluded_branches":[{"branch":"B2","reason":"설치 절차는 목적 밖이다"}]});
        let contradiction =
            decode_generated_outline(&both.to_string(), &mut llm::JsonRepair::default(), &labels)
                .err()
                .context("a branch both covered and excluded must not decode")?
                .to_string();
        assert!(contradiction.contains("B2"), "{contradiction}");
        assert!(
            contradiction.contains("excluded_branches"),
            "{contradiction}"
        );
    ...
    fn a_reading_past_the_requested_count_is_kept_when_it_is_valid_otherwise() -> Result<()> {
        ...
        let gaps = |count: usize, size: usize| -> Result<SourceBrief> {
            ...
            brief.uncertainties = (0..count)
                .map(|n| format!("{n}{}", "가".repeat(size)))
                .collect();
            Ok(brief)
        };
        ...
        let error = validate_brief(&mut gaps(ACCEPTED_UNCERTAINTIES + 1, 1)?, &sources, true)
            ...
            .to_string();
        assert!(
            error.contains(&(ACCEPTED_UNCERTAINTIES + 1).to_string()),
            "{error}"
        );
        ...
        let passages: Vec<Evidence> = (0..ACCEPTED_EVIDENCE_IDS + 1)
            .map(|n| {
                evidence(
                    &format!("/project/p{n}.py"),
                    &format!("def f{n}(): return {n}"),
                )
            })
            .collect();
        ...
        let error = validate_brief(&mut grouped(ACCEPTED_EVIDENCE_IDS + 1)?, &passages, true)
            ...
            .to_string();
        assert!(
            error.contains(&ACCEPTED_EVIDENCE_IDS.to_string()),
            "{error}"
        );
